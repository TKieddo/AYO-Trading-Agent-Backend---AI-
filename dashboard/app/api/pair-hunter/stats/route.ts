  ...
  try {
    ...
    for (const row of rows) {
      ...
      stats[asset] = {
        ...
        data_fail_count: Number(row.data_fail_count || 0),
        excluded_until: row.excluded_until || null,
        exclusion_reason: row.exclusion_reason || "",
        last_data_error: row.last_data_error || "",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const row = {
      ...
      data_fail_count: Number(stats.data_fail_count || 0),
      excluded_until: stats.excluded_until || null,
      exclusion_reason: String(stats.exclusion_reason || ""),
      last_data_error: String(stats.last_data_error || ""),
