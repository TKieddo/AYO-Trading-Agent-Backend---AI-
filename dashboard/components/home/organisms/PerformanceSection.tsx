  ...
  return (
    <div className="relative rounded-2xl bg-transparent p-4 md:p-6 overflow-hidden min-h-[400px] md:min-h-[600px]">
      <div className="relative z-10">
        ...
        <div className="inline-block rounded-lg border border-lime-400/30 p-3 mb-3 w-fit bg-black/20 backdrop-blur-sm">
          ...
          <div className="mb-1">
            ...
            <div className="flex items-center gap-1.5">
              <span className="px-2 py-0.5 bg-lime-400/20 border border-lime-400/30 rounded-full text-[10px] text-lime-300">{data.period}</span>
              <div className="flex items-center gap-0.5">
                <div className="w-1 h-1 bg-lime-400 rounded-full"></div>
                <span className="text-lime-400 text-[10px]">{data.change}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Tab Buttons */}
        <div className="flex gap-1 rounded-lg border border-lime-400/30 p-1 mb-4 w-fit bg-black/20 backdrop-blur-sm">
          <button
            onClick={() => setActiveTab("crypto")}
            className={`px-3 py-1 rounded-md text-xs font-medium transition-all duration-200 ${
              activeTab === "crypto"
                ? "bg-lime-400 text-black"
                : "bg-transparent text-lime-300/70 hover:bg-lime-400/10 hover:text-lime-400"
            }`}
          >
            Crypto
          </button>
        ...
        {currentStrategies && currentStrategies.length > 0 ? (
          <div className="flex flex-col gap-2 mb-6">
            {currentStrategies.map((strategy, index) => (
              <button
                key={index}
                className="inline-flex items-center gap-2 rounded-lg border border-lime-400/30 hover:border-lime-400 bg-black/20 backdrop-blur-sm hover:bg-black/30 transition-all duration-200 px-3 py-1.5 w-fit"
              >
                <span className="text-white font-semibold text-xs whitespace-nowrap">{strategy.pair}</span>
                <span className="px-1.5 py-0.5 bg-lime-400/30 border border-lime-400/50 text-lime-300 rounded-full text-[10px] font-medium whitespace-nowrap">
                  {strategy.profitPercent}
                </span>
                <span className="font-semibold text-lime-400 text-[10px] whitespace-nowrap">{strategy.profit}</span>
                <span className="text-lime-300/70 text-[10px] whitespace-nowrap">{strategy.performance}</span>
                <svg className="w-3 h-3 text-lime-400/60 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {data.metrics.map((metric, index) => (
              <div key={index} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-lime-300/70 text-xs">{metric.label}</div>
