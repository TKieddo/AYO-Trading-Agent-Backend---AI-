...
class LLMTrendStrategy(StrategyInterface):
    ...
    def _decide(self, context, assets):
        ...
        system_prompt = (
            ...
            "   - YOU must check TP/SL settings and close positions when TP is reached\n"
            ...
            "   - SIMPLE RULE: If PnL% >= TP%, close the position to take profit\n"
            "   - Example: If scalping_tp_percent = 5% and PnL% = 6.5%, close immediately (6.5% >= 5%)\n"
            "   - This applies to ALL positions, even if opened by a different strategy\n"
            "   - If take_profit_strict_enforcement is true, you MUST close when TP% is reached\n"
            "   - If take_profit_strict_enforcement is false, you can use market conditions but should still respect TP%\n"
            ...
            "- TP/SL sanity - USE INDICATORS FOR EXITS (Not Fixed High TPs!):\n"
            "  • CRITICAL: This is a TREND strategy - use TECHNICAL INDICATORS to determine exits, not fixed high TPs\n"
            ...
            "    - When TP is hit → Position closes automatically\n"
