...
class ScalpingScorer:
    """Scores pairs for scalping suitability — prefers medium ATR, not max heat."""
    
    def __init__(self, ideal_atr: float = None, max_atr: float = None):
        from src.config_loader import CONFIG
        self.ideal_atr = float(
            ideal_atr
            if ideal_atr is not None
            else CONFIG.get("pair_hunter_ideal_volatility", 3.0) or 3.0
        )
        self.max_atr = float(
            max_atr
            if max_atr is not None
            else CONFIG.get("pair_hunter_max_volatility", 6.0) or 6.0
        )
        self.min_atr = float(CONFIG.get("pair_hunter_min_volatility", 1.5) or 1.5)
    ...
    def score_pairs(self, market_data: Dict, atr_data: Dict[str, float]) -> List[ScoredPair]:
        ...
        for symbol, data in market_data.items():
            ...
            if atr_pct < self.min_atr or atr_pct > self.max_atr:
                continue
            ...
            # Less weight on volatility heat; more on volume + controlled trend
            total_score = (
                vol_score * 0.25 +
                volume_score * 0.35 +
                trend_score * 0.25 +
    ...
    def _calc_volatility_score(self, atr_pct: float) -> float:
        # Peak at ideal_atr; fade outside; hard reject handled by max_atr filter
        if atr_pct < self.min_atr:
            return 0.05
        span = max(self.ideal_atr - self.min_atr, self.max_atr - self.ideal_atr, 0.5)
        return max(0.1, 1.0 - abs(atr_pct - self.ideal_atr) / span)
